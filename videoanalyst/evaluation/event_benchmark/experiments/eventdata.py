...
class ExperimentEVENT(object):
            ...
            for s in range(start_idx, end_idx):
                ...
                if save_video:
                    ...
                    for ith, (img_file, pred) in enumerate(zip(img_files, boxes)):
                        imname=img_file.split('/')[-1].split('.')[0]
                        sname=img_file.split('/')[-3]
                        n="{:0>4}".format(str(int(imname)-1))
                        img_file=f'/data2/DataSet/VisEvent/test_subset/{sname}/vis_imgs/frame{n}.bmp'
    ...
    def report(self, tracker_names, plot_curves=True):
        ...
        if self.subset == 'test':
            ...
            for name in tracker_names:
                ...
                ious = {}
                times = {}
                prec_curve = np.zeros((len(self.dataset), self.nbins_ce))
                ...
                for s, (_, anno, meta) in enumerate(self.dataset):
                    ...
                    center_errors = center_error(boxes[0][1:], anno[1:])
                    _, prec_curve[s] = self._calc_curves(seq_ious, center_errors)
                ...
                # print(f'ao:{ao}\tsr50:{sr50}\tsr75:{sr75}\tPrecision:{prec_score}\t')
    ...
    def plot_curves(self, report_files, tracker_names, extension='.png'):
        ...
        prec_file = os.path.join(report_dir, 'precision_plot' + extension)
        key = 'overall'
