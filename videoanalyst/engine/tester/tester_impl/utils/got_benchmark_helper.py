...
# from videoanalyst.engine.tester.tester_impl.utils.vision import visualize_grid_to_grid_with_cls
...
class PipelineTracker(object):
    ...
    def init(self, image_pos: np.array, image_neg, box, snn_state_first):
        ...
        box : np.array or List
            ...
            formate: (x, y, w, h)
        """
        functional.reset_net(self.pipeline._model)
    ...
    def track(self, img_files_pos: List, img_files_neg, box, visualize: bool = False):
        ...
        for f, img_file in enumerate(img_files_pos):
            ...
            # image = Image.open(img_file)
            # visualize_grid_to_grid_with_cls(get_local.cache['Attention.forward'][-1][0][-1][:][:], 500, image)
            if visualize:
