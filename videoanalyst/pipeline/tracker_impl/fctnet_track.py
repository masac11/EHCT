...
class EHCTTracker(PipelineBase):
    r"""
    ...
    def __init__(self, *args, **kwargs):
        super(EHCTTracker, self).__init__(*args, **kwargs)
        self.update_params()
    ...
    def feature(self, im_pos: np.array, im_neg, target_pos, target_sz, avg_chans=None):
        """Extract feature
        ...
        with torch.no_grad():
            ...
            features = self._model(data_pos, data_neg, phase=phase)

        return features, im_z_crop_pos + im_z_crop_neg, avg_chans

    def init(self, im_pos, im_neg, state, snn_state_first=None):
        ...
        features, im_z_crop, avg_chans = self.feature(im_pos, im_neg, target_pos, target_sz)
    ...
    def track(self,
        ...
        with torch.no_grad():
            ...
            # timea = time.time()
            score, box, cls, ctr, extra = self._model(data_pos, data_neg,
                                                      *features,
                                                      phase=phase_track)
    ...
    def update(self, im_pos, img_neg, snn_state=None, state=None):
        ...
        target_pos, target_sz = self.track(im_pos, img_neg,
                                           target_pos_prior,
                                           target_sz_prior,
                                           features,
                                           update_state=True)
