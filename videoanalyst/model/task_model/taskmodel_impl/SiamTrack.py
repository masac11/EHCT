# -*- coding: utf-8 -*
# 两个分支都先用resnet50提取特征，然后在后面融合，最后TSFF
...
class SiamTrack(ModuleBase):
    ...
    def forward(self, *args, phase=None):
        ...
        # used for template feature extraction (normal mode)
            ...
            target_img_pos, target_img_neg = args
            if self._hyper_params["trt_mode"]:
                # extract feature with trt model
                out_list = self.trt_fea_model(target_img_pos)
            else:
                fea_base = self.basemodel(target_img_pos, target_img_neg, first_seq=True)
                fea_z, trans_z = self.transfor(fea_base, first=True)
                # trans_z_sig, trans_z_lowres = self.transfor(tem_pos, tem_neg)
                f_z = self.atten(fea_z, trans_z, fea_base)
                # template as kernel
        ...
        # used for tracking one frame during test
            ...
            if len(args) == 4:
                search_img_pos, search_img_neg, c_z_k, r_z_k = args
                if self._hyper_params["trt_mode"]:
                    c_x, r_x = self.trt_track_model(search_img_pos)
                else:
                    # backbone feature
                    fea_x = self.basemodel(search_img_pos, search_img_neg, first_seq=False)
                    tem_fea_x, spa_fea_x = self.transfor(fea_x)
                    f_x = self.atten(tem_fea_x, spa_fea_x, fea_x)
                    c_x = self.c_x(f_x)
                    r_x = self.r_x(f_x)
            elif len(args) == 5:
